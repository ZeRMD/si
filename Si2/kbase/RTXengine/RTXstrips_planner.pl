...
solve(Goals,W0, W3, Forbidden, Plan, Domain):-
    ...
    achieves(Goal, ListOfActions),
    member(Action, ListOfActions),
/*
    (   ListOfActions \= [],
         member(Action, ListOfActions), !
    ;
         clause(strips(List_Spec), _),
         member(add  Add, List_Spec),
         member(Goal, Add),
        Action = strips(List_Spec)
    ),
*/
    ...
    (
      ...
      ;
       ...
       subtract_set(List_Clean, [wait(_), clear(_)], List_Clean_no_waits),
       subset(List_Clean_no_waits, W1)
    ),
    */
...
member_of_world(Fact):-
  world(World, _),
  member(Fact, World).


member_of_goals(Fact):-
  goals(Goals),
  member(Fact, Goals).
...
achieves(G, ListOfActions):-
    findall( (Priority, strips(List_spec)),
    (
      clause(strips(List_spec), _BODY),
      member(add AddList, List_spec),
      (   member(priority(Priority), List_spec)
          ;
          \+ member(priority(_), List_spec),
          Priority = 0
      ),
    ...
    List),
    sort(1, '@>=', List, List2),
    findall(Action,  member( (_P, Action), List2), ListOfActions).
...
do(Action, W1, W4, _PreviousPlan):-
   ...
   (
       member(effect EffectList, Spec_List),
       subtract_set(EffectList, W3, EffectList2),
       %writeln(subtract_set(EffectList, W3, EffectList2)),
       append(W3,EffectList2, W4),
       !;
       W4 = W3
   ).
